...
<div class="container">
      <h1>Themes</h1>
      <p>What Themes we have approved, as always, see the <code>readme.tx</code> that comes with each Theme for any specific instructions.</p>
</div>
<div class="container">
   <div class="row">
        <?php
         ...
         * @approved ( 1 = Yes, 0 = No )
        */
        $query = "SELECT * from mc_repository WHERE type = 2 AND approved = 1";
        $result = $db->query($query);
        $nums = $db->rows($result);
        if($nums == 0)
        {
          echo '<p>No Themes exist or have been approved</p>';
        }
        else
        {
           while($plugins = $db->fetch_assoc($result))
           {
              $theme_title = $plugins['title'];
              $theme_text = $plugins['content'];
              $theme_category = $plugins['category'];
              $theme_subcat = $plugins['sub_category'];
              $theme_creator = $plugins['creator'];
              $theme_version = $plugins['version'];
              $theme_slug = $plugins['slug'];
              $file_link = '/files/themes/'.$theme_slug.'-'.$theme_version.'.zip';
              ...
              echo '<div class="card">';
              echo '<img class="card-img-top" src="/mc-includes/images/themes/'.$plugin_slug.'.png" alt="'.$plugin_title.'" height="200" width="100">';
         ...
         {
            echo '<br><p><a href="/dev/theme_upload">Developer Upload</a></p>';
         }
?>
   </div>
</div>
